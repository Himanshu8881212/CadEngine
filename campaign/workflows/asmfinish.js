...
- SCENE EXPORT (updated 2026-09-05): the merged scene STL of an assembled attitude is a DIAGNOSTIC export — since the 2026-08-23 engine (round 4) `asm_export` writes it with `scene: true` and reports designed seats / interference as `cross_instance_self_intersections` instead of refusing (the 2026-08-10..23 kernels refused it; that window is over). What stays strict is each PER-INSTANCE part file. And `asm_export_step` never merges triangles at all — it writes one STEP PRODUCT per instance and therefore cannot hit any merge-side refusal (graham_deadbeat_escapement F7: three stages shipped no assembly CAD because the old note read as "the attitude cannot be exported"). Every MEASURE gate runs first either way; if any export op still refuses, record the verbatim refusal as an ATTEMPTED receipt and proceed on the interference/clearance numbers.
