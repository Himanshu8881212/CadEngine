...
- status: fixed — `require` `within` takes `[lo, hi]` (or `{target, abs|percent}`); `require_within` repro; brief §3 documents the shape
...
- severity: major
- surface: export_stl
- status: fixed — the bore with several cutters exports exact at every pose (`uphill_cone_half` re-run: stl/3mf exact)
...
- status: fixed — `wall_thickness {exclude_wedge_deg}` keeps the obtuse rim out of `thin_area` (reported as `thin_area_wedge`)
...
- **F1** — F1: within shape.
- **F2** — F2: position sensitivity gone.
- **F3** — F3: rim excluded.
