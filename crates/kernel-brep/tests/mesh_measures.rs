...
fn radial_extent_is_exact_on_banded_boxes_and_pierced_caps() {
	...
	let (rmin_band, rmax_band) = m.radial_extent(Vec3::ZERO, Vec3::Z, Some((10.0, 30.0))).expect("band intersects the box");
	...
	assert!(
		(rmin_band - 10.0).abs() < 1e-9 && (rmax_band - d).abs() < 1e-9 && rmin_full < 1e-9 && empty.is_none(),
...
fn support_report_names_where_its_bridges_and_steep_areas_are() {
	...
	let bridge_at_slab = rep.bridge_patches.first().map(|(span, at)| *span > 1.0 && (at.z - 20.0).abs() < 0.5).unwrap_or(false);
	...
	let steep_named =
		rep2.steep_area > 10.0 && !rep2.steep_exemplars.is_empty() && rep2.steep_exemplars.iter().all(|p| p.z > 1.0 && p.z < 10.5);
