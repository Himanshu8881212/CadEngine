#!/usr/bin/env python3
# LMCAD forwarding shim — render_views.py moved to tools/publish/render_views.py on 2026-09-02 (the
...
# `python3 tools/render_views.py job.json` keeps working for every campaign run_all.sh,
# CI job and the MCP server; imported as a module it hands back the real module.
# Edit the real file, never this one.
import os
import sys

_target = os.path.join(os.path.dirname(os.path.abspath(__file__)), "publish", "render_views.py")
