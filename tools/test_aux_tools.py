...
def run_tool(tool, *args):
	p = subprocess.run([PY, os.path.join(HERE, tool), *args], capture_output=True, text=True)
...
def test_bom_audit_example_job_is_wellformed(tmp):
	p = subprocess.run([PY, os.path.join(HERE, "bom_audit.py"), "--example"],
...
def test_help_never_crashes():
	...
	for tool in ("render_sheet.py", "analysis_sheet.py", "assembly_doc.py",
		...
		p = subprocess.run([PY, os.path.join(HERE, tool), "--help"],
		                   capture_output=True, text=True)
