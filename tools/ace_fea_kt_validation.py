...
def solve(N, keep_field=None):
	...
	rv = subprocess.run([PY, os.path.join(REPO, "tools", "voxelize_stl.py"), os.path.join(OUT, "_vjob.json")],
	                    capture_output=True, text=True)
	...
	rf = subprocess.run([PY, os.path.join(REPO, "tools", "ace_fea_runner.py"), os.path.join(OUT, "_fjob.json")],
	                    capture_output=True, text=True)
...
def render_analysis_sheet():
	...
	rs = subprocess.run([PY, os.path.join(REPO, "tools", "analysis_sheet.py"), jp],
	                    capture_output=True, text=True)
