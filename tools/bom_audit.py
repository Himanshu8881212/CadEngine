#!/usr/bin/env python3
# LMCAD forwarding shim — bom_audit.py moved to tools/publish/bom_audit.py on 2026-09-02 (the
...
# `python3 tools/bom_audit.py job.json` keeps working for every campaign run_all.sh,
# CI job and the MCP server; imported as a module it hands back the real module.
# Edit the real file, never this one.
import os
import sys

_target = os.path.join(os.path.dirname(os.path.abspath(__file__)), "publish", "bom_audit.py")
