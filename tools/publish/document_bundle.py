...
TOOLS = str(_layout.TOOLS)  # tools/ root; sibling scripts are resolved through _layout.find_tool
...
def run_tool(script, job_obj, job_path, receipt_path, cwd):
	...
	with open(job_path, "w") as f:
		...
		f.write("\n")
	out = subprocess.run([sys.executable, str(_layout.find_tool(script)), os.path.abspath(job_path)],
	...
	try:
		...
		for e in ar.REGISTRY:
			if isinstance(e, dict) and e.get("file"):
				by_script[os.path.basename(str(e["file"])).replace(".py", "")] = str(e.get("claimed_tier"))
