...
- **Receipt contract**: the LAST non-empty stdout line of every runner is one
  JSON object; logging goes to stderr. The ACE-bridge runners (`ace_fea`,
  `modal`, `buckling`) exit 0 even on failure — the JSON line is the contract.
  The in-house runners (`thermal`, `contact`, `fatigue`) exit **1** on failure
  because they double as shell gates; each card says which it is.
