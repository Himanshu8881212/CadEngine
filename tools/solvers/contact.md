...
```
Outputs: `curve.npy` — float64 `(n_steps+1, 16)` C-order, columns in receipt `curve_columns`: lambda, obstacle_travel_mm, **insertion_force_n**, total_normal_force_n, tip_ux/uy_mm, max_disp_mm, max_abs_stress_mpa, max_penetration_mm, n_contact_nodes, newton_iters, residual_norm_n, **reaction_fx_n / reaction_fy_n / reaction_mz_nmm** (reactions summed over the ramped = prescribed-displacement nodes per step; 0.0 when none) and **tip_reaction_n** (the reaction at the node named "tip" along its ramped translational dof; NaN under load control). Receipt also carries `nonlinear` and `linear` side by side (so the linear over-prediction is visible), `reactions` (one row per support node at the final step: `{node, dofs_constrained, ramped, prescribed, fx_n, fy_n, mz_nmm}`, convention in `reaction_convention` = force BY the support ON the beam; for a prescribed displacement this is the actuator force — `tip_reaction_n` is the scalar convenience and `linear.reactions` / `linear.tip_reaction_n` the small-displacement answer), `path_max` (worst stress/strain over the WHOLE path — a latch that springs back ends unstressed), `insertion{peak_force_n, peak_at_travel_mm, min_force_n, final_force_n, max_penetration_mm}`, `nodes_final_mm`, and per-step `convergence.per_step` with iterations, residual history, `converged_by`, reactions (`reactions_n` per dof and `reactions_by_node`) and `equilibrium_residual_n`. Failure = `{ok:false, error}` + **exit 2** for a refusal (JobError / ConvergenceError) and **exit 1** for a broken request (the shared `tools/_receipt.py` contract; the negative controls pin it).
...
| 1b prescribed-displacement reaction (2026-09-02) | tip pushed to d = 0.01 L by a ramped support: `F = 3 EI d / L^3`, root `+F`, `M_root = F L` | nonlinear `tip_reaction_n` +1.05e-4 rel (the alpha = 0.03 elastica correction), linear 9.9e-12 rel; root balance 1e-9-class; curve columns == receipt; load control -> null / NaN | nonlinear within 2%; linear <= 1e-9; \|fy_root + fy_tip\|/F <= 1e-9; mz_root within 2% of F L |
