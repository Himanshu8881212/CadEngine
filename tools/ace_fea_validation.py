#!/usr/bin/env python3
# LMCAD forwarding shim — ace_fea_validation.py moved to tools/validation/ace_fea_validation.py on 2026-09-02 (the
...
# `python3 tools/ace_fea_validation.py job.json` keeps working for every campaign run_all.sh,
# CI job and the MCP server; imported as a module it hands back the real module.
# Edit the real file, never this one.
import os
import sys

_target = os.path.join(os.path.dirname(os.path.abspath(__file__)), "validation", "ace_fea_validation.py")
