#!/usr/bin/env python3
# LMCAD forwarding shim — analysis_sheet.py moved to tools/publish/analysis_sheet.py on 2026-09-02 (the
...
# `python3 tools/analysis_sheet.py job.json` keeps working for every campaign run_all.sh,
# CI job and the MCP server; imported as a module it hands back the real module.
# Edit the real file, never this one.
import os
import sys

_target = os.path.join(os.path.dirname(os.path.abspath(__file__)), "publish", "analysis_sheet.py")
