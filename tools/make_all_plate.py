#!/usr/bin/env python3
# LMCAD forwarding shim — make_all_plate.py moved to tools/publish/make_all_plate.py on 2026-09-02 (the
...
# `python3 tools/make_all_plate.py job.json` keeps working for every campaign run_all.sh,
# CI job and the MCP server; imported as a module it hands back the real module.
# Edit the real file, never this one.
import os
import sys

_target = os.path.join(os.path.dirname(os.path.abspath(__file__)), "publish", "make_all_plate.py")
