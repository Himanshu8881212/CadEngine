#!/usr/bin/env python3
# LMCAD forwarding shim — ace_fea_tet_runner.py moved to tools/analyzers/ace_fea_tet_runner.py on 2026-09-02 (the
...
# `python3 tools/ace_fea_tet_runner.py job.json` keeps working for every campaign run_all.sh,
# CI job and the MCP server; imported as a module it hands back the real module.
# Edit the real file, never this one.
import os
import sys

_target = os.path.join(os.path.dirname(os.path.abspath(__file__)), "analyzers", "ace_fea_tet_runner.py")
