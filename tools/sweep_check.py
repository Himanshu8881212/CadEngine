#!/usr/bin/env python3
# LMCAD forwarding shim — sweep_check.py moved to tools/analyzers/sweep_check.py on 2026-09-02 (the
...
# `python3 tools/sweep_check.py job.json` keeps working for every campaign run_all.sh,
# CI job and the MCP server; imported as a module it hands back the real module.
# Edit the real file, never this one.
import os
import sys

_target = os.path.join(os.path.dirname(os.path.abspath(__file__)), "analyzers", "sweep_check.py")
